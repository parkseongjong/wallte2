...
<?
    ...
    if ( $tx_cd == "TX00" )
    {
		...
		$deadline_use_date = date('Y-m-d H:i:s', strtotime($now_date.'+60 days'));

		$db = getDbInstance();
		$db->where('tno', $tno);
		$updateArr = [] ;   	
		$updateArr['order_status']='0';
		$updateArr['bank_insert_name']= $remitter;		
		//$updateArr['udate'] = date('Y-m-d H:i:s');
		$updateArr['udate'] = $now_date;
		$aa=$db->update('kcp_order', $updateArr);

		if ( $aa ) { // 성공시
			$order1 = [] ;
			$db->where('tno', $tno);   
			$order1['gstatus']="0";				
			$order1['bank_insert_name']= $remitter;
			//$order1['bank_at'] = date('Y-m-d H:i:s');
			$order1['bank_at'] = $now_date;
			$order1['deadline_use'] = $deadline_use_date;
			$order_last_id = $db->update ('coupon_result', $order1);
			if ( !$order_last_id ) {
				$pay_result = '1111';
			}
		} else {
    ...
    /* ============================================================================== */
?>
<html><body><form><input type="hidden" name="result" value="<?php echo $pay_result; ?>"></form></body></html>