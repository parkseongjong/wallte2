...
/*
if($_POST['phone'] != '+821066253606'&& $_POST['phone'] != '+821094166070' && $_POST['phone'] != '+821031235615'){
    $_SESSION['login_failure'] = $langArr['login_fail_msg3'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST')
{
    ...
    try{
        if ($cookieAutoLoginKey) {
            ...
            if (count($cookieAutoLoginKey) != 2) {
                throw new Exception('비정상적인 요청 입니다.(3) 자동로그인을 다시 설정해 주세요.', 9999);
            }
            if ($cookieAutoLoginKey[0] != 'autologin') {
                throw new Exception('비정상적인 요청 입니다.(4) 자동로그인을 다시 설정해 주세요.', 9999);
            }
            ...
            $memberInfo = $walletDb->createQueryBuilder()
                ...
                ->execute()->fetch();
            if (!$memberInfo) {
                throw new Exception('비정상적인 요청 입니다.(1) 자동로그인을 다시 설정해 주세요.', 9999);
            }
            ...
            //쿠키 passwd 삭제
            $targetUnixDatetime = strtotime($memberInfo['ua_datetime'] . '+15 days');
            $nowDatetime = strtotime($utill->getDateSql());

            if ($nowDatetime > $targetUnixDatetime) {
                ...
                throw new Exception('만료 된 KEY 값 입니다.(15일간 유효) 자동로그인을 다시 설정해 주세요.', 9999);
            }

            $tempMemberInfo = $walletDb->createQueryBuilder()
                ->select('*')
                ->from('admin_accounts')
                ->where('id = ?')
                ->andWhere('email = ?')
                ...
                ->execute()->fetchAll();
            if(!$tempMemberInfo){
                throw new Exception('KEY 값 정보가 일치하지 않습니다. 자동로그인을 다시 설정해 주세요.', 9999);
            }

            //key 값 생성 변경,
            $hash = hash('sha256','@autoLoginHash|'.$tempMemberInfo[0]['id'], false);
            if ($hash != $cookieAutoLoginKey[1]) {
                //key 값과 session id 값 ip값 세개다 일치 (올드 버전 확인)
                $hash = hash('sha256', $memberInfo['ua_session_id'] . $memberInfo['ua_ip'], false);
            }
            if ($hash != $cookieAutoLoginKey[1]) {
                throw new Exception('비정상적인 요청 입니다.(2) 자동로그인을 다시 설정해 주세요.', 9999);
            }

            //디바이스 고유 id를 특정 할 방법이 아직 없어서... 로그인 한 id를 기준으로... 자동로그인을 처리 한다.
            $row = $tempMemberInfo;
            unset($tempMemberInfo);
        }
    }
    catch (Exception $e){
        ...
        $data_to_login['msg'] = $e->getMessage().'/'.print_r($cookieAutoLoginKey,true);
        $data_to_login['ip'] = $userip;
        $login_logs_id = $db->insert('login_logs', $data_to_login);

        // 일치하는 아이디 없음
        $_SESSION['login_failure'] = $e->getMessage();
        header('Location:login.php');
        exit();
    }
    //자동로그인 인지 아닌지 판별. END
    ...
    if(!$row){
        ...
        $row = $walletDb->createQueryBuilder()
            ->select('id, passwd, passwd_new, passwd_salt, passwd_datetime, auth_local_code, register_with, wallet_phone_email')
        ...
        if(!$row){
            //salt 값을 가져 와야해서 email로 조회
            $memberInfo = $walletDb->createQueryBuilder()
                ->select('passwd, passwd_new, passwd_salt, passwd_datetime, auth_local_code, register_with, wallet_phone_email')
                ->from('admin_accounts_sleep')
                ->where('email = ?')
                ->andWhere('passwd_new is not NULL')
                ->andWhere('passwd_datetime is not NULL')
        ...
        if($row){
            ...
            ob_end_clean();
            $templates = new plateTemplate(WALLET_PATH.'/skin/sleepUser', 'php');
...
*/
    ...
    if (count($row) >= 1) {
        ...
        try{
            if ($remember >= 1) {
                //모바일은 session id가 다시 재발급 되는 것 같음... (자동으로 비워준다거나 그런가...?)
                ...
                //키 값 해시 변경. 기기마다 특정 할 방법이 없어서.... 해결 될 때 까진 이렇게 처리.
                $hash = hash('sha256','@autoLoginHash|'.$row[0]['id'], false);
        ...
        //PC 접근 제한, 20.09.04 아래 고유 id는 접근 가능
        //if ( $row[0]['id'] != '5885' ) {
        //17299 병욱 과장님, 17417  성종과장님, 17356 아산 베리
        if ($row[0]['id'] != '17553' && $row[0]['id'] != '16958' && $row[0]['id'] != '6423' && $row[0]['id'] != '12636' &&  $row[0]['id'] != '17417' && $row[0]['id'] != '11863' && $row[0]['id'] != '2297' && $row[0]['id'] != '6112' && $row[0]['id'] != '10086' && $row[0]['id'] != '12122' && $row[0]['id'] != '17299' && $row[0]['id'] != '17294'&& $row[0]['id'] != '17356') {
            // 10086 : �Ͻ�
        ...
        if ( empty($row[0]['last_login_at']) || ( $last_login_date_tmp && $last_login_date_tmp != date("Y-m-d") ) ) { // �α����� �ѹ��� ���߰ų� ������ �α��� ��¥�� ������ �ƴϸ� ����, 20.10.16
            ...
            $data_to_login['msg'] = print_r($cookieAutoLoginKey,true);
            $data_to_login['ip'] = $userip;
            //	$db = getDbInstance();
            $login_logs_id = $db->insert('login_logs', $data_to_login);
        }
