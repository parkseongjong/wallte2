...
//                        }
                        ...
                        if($interval->y < 14){
                            $_SESSION['failure'] = $langArr['commonApiStringDanger100'];
                            $_SESSION['auth_re_r'] = 'fin';

                            $data_auth_err = [];
                            $data_auth_err['type'] = 'auth';
                            $data_auth_err['cause'] = 'decrypt';
                            $data_auth_err['message'] = $langArr['commonApiStringDanger100'];
                            $data_auth_err['browser_infos'] = $browser_infos;
                            $data_auth_err['page_url'] = $page_url;
                            $data_auth_err['user_ip'] = $user_ip;
                            $data_auth_err['created_at'] = date("Y-m-d H:i:s");
						...
						if ($auth_phone_no != '' ) {
							$db3 = getDbInstance();
							$db3->where("id", $param_opt_3);
							$row3 = $db3->getOne('admin_accounts');
							if ( $row3['register_with'] == 'phone' &&  stristr($row3['email'], '+8210') == true && strlen($row3['email']) == '13' && str_replace('+8210', '010', $row3['email']) != $auth_phone_no ) {
								// 이메일과 휴대폰인증한 번호가다름 => 실패

								$_SESSION['failure'] = $langArr['auth_err_different_email'];
								$_SESSION['auth_re'] = 'fin';
								fn_logSave('Personal Identification Error : '.$langArr['auth_err_different_email']);

								$data_auth_err = [];
								$data_auth_err['type'] = 'auth';
								$data_auth_err['cause'] = 'different';
								$data_auth_err['message'] = $langArr['auth_err_different_email'];
								$data_auth_err['browser_infos'] = $browser_infos;
								$data_auth_err['page_url'] = $page_url;
								$data_auth_err['user_ip'] = $user_ip;
								$data_auth_err['created_at'] = date("Y-m-d H:i:s");
				...
				$data_auth_err['type'] = 'auth';
				$data_auth_err['cause'] = 'decrypt';
				$data_auth_err['message'] = $e->getMessage().$e->getLine().$e->getFile();
				$data_auth_err['browser_infos'] = $browser_infos;
				$data_auth_err['page_url'] = $page_url;
				$data_auth_err['user_ip'] = $user_ip;
				$data_auth_err['created_at'] = date("Y-m-d H:i:s");
