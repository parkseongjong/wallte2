...
$return_page = 'send_other.php?token='.$token;
...
$getTokenListData = $db->where("token_name", $token)->getOne('tokenlist');
$monthlyTransferLimit = $getTokenListData['monthly_transfer_limit'];
...
$addressList = $db->get('user_withdrawal_addresses');
    ...
    $todayTime = time();
    $firstDayOfMonthTime = strtotime(date("Y-m-1 00:00:00"));
   
 
	$curl = curl_init();
	$ethUrl = "http://api.etherscan.io/api?module=account&action=tokentx&contractaddress=".$tokenContractAddress."&address=".$walletAddress."&sort=desc&apikey=".$ethApiKey;
	...
	$err = curl_error($curl);
	curl_close($curl);
	$getResultDecodeCurl = json_decode($response,true);
	$getRecordsCurl = $getResultDecodeCurl['result']; 

	$monthlyRecordCount = 0;
	if(!empty($getRecordsCurl)){
		foreach($getRecordsCurl as $getRecordsCurlSingle){
			if($getRecordsCurlSingle['timeStamp']>=$firstDayOfMonthTime && $getRecordsCurlSingle['timeStamp']<=$todayTime){
				$monthlyRecordCount++;
			}
		}
	}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if($monthlyRecordCount>=$monthlyTransferLimit){

		$_SESSION['failure'] .= !empty($langArr['monthly_trasfer_limit_exceed']) ? $langArr['monthly_trasfer_limit_exceed'] : 'Monthly Trasfer Limit Exceed';
	...
	if(!empty($feeTransactionId)){
		...
		try {
			$otherTokenContractInner = $web3Instance->innerContract($web3->provider, $testAbi);
			$otherTokenContractInner->at($tokenContractAddress)->send('transfer', $toAccount, $amountToSend, [
				'from' => $fromAccount,
				'gas' => '0x186A0',   //100000
				'gasprice'=>$gasPriceInWei
							...
							<div class="boxed bg--secondary boxed--lg boxed--border">
								...
								<form class="form" action="set_transferpw_frm_send.php" method="post"  id="customer_form" enctype="multipart/form-data">
									...
									<div class="col-md-12 btn_area">
										<?php if($monthlyRecordCount>=$monthlyTransferLimit){ ?>
											<input name="submit" class="btn" id="confirm_modal" value="<?php echo !empty($langArr['monthly_trasfer_limit_exceed']) ? $langArr['monthly_trasfer_limit_exceed'] : "Monthly Trasfer Limit Exceed"; ?>" type="submit" disabled />
										<?php } else { ?>
