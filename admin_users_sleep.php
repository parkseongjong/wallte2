...
// BBS 공통 함수 추후 분리하기 .. START By.OJT
$select = array(
    'A.id',
    'A.account_type2',
    'B.name',
    ...
    'B.auth_phone',
    'A.transfer_approved',
    'A.wallet_address_change',
    'A.wallet_change_apply',
    'B.device',
    'B.devId',
    'B.devId2',
    'B.devId3',
...
$pageNationInfo = $builderParams = array();
$pageNationInfo['page'] = $page;
$pageNationInfo['pageRow'] = $pagelimit;

$listInfoBuilder = $walletDb->createQueryBuilder()
    ->select($select)
    ->from('admin_accounts','A')
    ->innerJoin('A','admin_accounts_sleep','B','A.id = B.id');

if ( !empty($date1) ) {
    $listInfoBuilder->andWhere('A.created_at >= ?');
    array_push($builderParams,$date1.' 00:00:00');
}
if ( !empty($date2) ) {
    $listInfoBuilder->andWhere('A.created_at <= ?');
    array_push($builderParams,$date2.' 23:59:59');
}

//검색어 있는 경우.. 처리..
if($search_string) {
    if (preg_match("/[\xE0-\xFF][\x80-\xFF][\x80-\xFF]/", $search_string)){ // utf-8인 경우
        foreach (['B.name','B.auth_name'] as $value){
            $listInfoBuilder->orWhere('convert('.$value.' using utf8) = ?');
            array_push($builderParams,$search_string);


        }
    }
    else {
        //foreach (['B.email','B.phone','A.wallet_address','A.virtual_wallet_address','A.wallet_address_change','B.name','B.auth_name','B.auth_phone','B.n_phone'] as $value){
        foreach (['B.email','B.phone','B.name','B.auth_name','B.auth_phone','B.n_phone'] as $value){
           if($value == 'B.email'){
                $listInfoBuilder->orWhere('B.email'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

            }else if($value == 'B.phone'){
                $listInfoBuilder->orWhere('B.phone'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'A.wallet_address'){
                $listInfoBuilder->orWhere('A.wallet_address'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'A.virtual_wallet_address'){
                $listInfoBuilder->orWhere('A.virtual_wallet_address'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'A.wallet_address_change'){
                $listInfoBuilder->orWhere('A.wallet_address_change'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'B.name'){
                $listInfoBuilder->orWhere('B.name'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'B.auth_name'){
                $listInfoBuilder->orWhere('B.auth_name'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'B.auth_phone'){
                $listInfoBuilder->orWhere('B.auth_phone'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);

           }else if($value == 'B.n_phone'){
                $listInfoBuilder->orWhere('B.n_phone'.' like '.'"%'.$search_string.'%"'.'');
               array_push($builderParams,$search_string);
    ...
    $walletLogger->info('관리자 모드 > 휴면 사용자 목록 > 사용자 목록 > 검색 /검색어:'.$search_string.'/검색 조건:'.$filter_col,['admin_id'=>$walletLoggerUtil->getAdminSession(),'user_id'=>0,'url'=>$walletLoggerUtil->getUrl(),'action'=>'S']);
}
else{
    $walletLogger->info('관리자 모드 > 휴면 사용자 목록 > 사용자 목록 > 조회',['admin_id'=>$walletLoggerUtil->getAdminSession(),'user_id'=>0,'url'=>$walletLoggerUtil->getUrl(),'action'=>'S']);
}

if(count($builderParams) > 0){
    $listInfoBuilder->setParameters($builderParams);
}
...
if($pageNationInfo['totalPage'] == 0){
    //$resultData = false;
    //레거시에 맞춰서.. 결과가 없을 땐 비어있는 array 리턴.
    $resultData = array();
}
else{
    $resultData  = $listInfoBuilder->orderBy($filter_col,$order_by)->setFirstResult($pageNationInfo['fromRecord'])->setMaxResults($pageNationInfo['pageRow'])->execute()->fetchAll();
}
// BBS 공통 함수 추후 분리하기 .. END By.OJT
...
?>
    ...
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-6">
                <h1 class="page-header"><?php echo !empty($langArr['registered_users_sleep']) ? $langArr['registered_users_sleep'] : "Registered Sleep Users"; ?></h1>
            </div>
        ...
        <div class="tab-content">
            <div id="user_first" class="tab-pane fade in active">
                        ...
                        foreach ($resultData as $row) {
                            ...
                            <tr>
                                ...
                                <td rowspan="2" class="td_cls1">
                                    ...
                                    <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="name" data-user-data-type="S"><?php echo $walletMasking->reset()->pushNameMask()->getMasked(htmlspecialchars($row['lname']).htmlspecialchars($row['name'])); ?></span>
                                    ...
                                    <strong>IP:</strong>
                                    <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="ip" data-user-data-type="S">
                                ...
                                <td rowspan="2">
                                    <?php if ( $row['account_type2'] == $con_exchange_type_value ): ?>
                                        <?php if(!empty($row['external_phone'])): ?>
                                            <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="externalPhone" data-user-data-type="S"><?php echo $walletMasking->reset()->pushPhoneMask('other')->getMasked($row['external_phone']); ?></span>
                                        <?php endif; ?>
                                    <?php else: ?>
                                        <?php if($row['email'] && $row['register_with']=='email'): ?>
                                            <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="email" data-user-data-type="S">
                                        <?php echo $walletMasking->reset()->pushEmailMask()->pushUniversalIdMask()->getMasked(htmlspecialchars($row['email'])) ?>
                                        <br/>
                                    </span>
                                        <?php endif; ?>
                                        <?php if($row['phone']): ?>
                                            <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="phone" data-user-data-type="S">
                                        ...
                                        <?php if($row['id_auth'] == 'Y'): ?>
                                            ...
                                            <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="name" data-user-data-type="S">
                                            <?php echo $walletMasking->reset()->pushNameMask()->getMasked(htmlspecialchars($row['auth_name'])); ?>
                                        </span>
                                            <br/>
                                            <span class="maskingArea" data-id="<?php echo $row['id'] ?>" data-type="authPhone" data-user-data-type="S">
                                ...
                                <td rowspan="2">
                                    ...
                                    <a href="edit_users.php?admin_user_id=<?php echo $row['id']?>&operation=edit<?php if ( !empty($filterData) ) { echo '&'.http_build_query($filterData); } ?>" class="btn btn-primary"><span class="glyphicon glyphicon-edit"></span></a>
                ...
                <div class="text-center">
                    ...
                    echo new_set_page_list($currentPage, '', $pageNationInfo['totalPage'], $get_infos, '10'); // config/new_config.php
                    ?>
