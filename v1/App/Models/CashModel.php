...
class CashModel extends BaseModel
{
     ...
     */
    public function getCashCount($bind)
    {
        return $this->walletDb->createQueryBuilder()
            ->select('count(0) as cnt')
            ->from('tb_user_cash_log', 'cl')
            ->where('cl_user_id = :user_id')
            ->andWhere('cl_created >= :s_date')
            ->andWhere('cl_created <= :e_date')
     ...
     * @throws \Doctrine\DBAL\Exception
     */
    public function getCashList($bind)
    {
        $fields = [
            'code',
            'quantity',
        ...
        $fields = static::addPrefixFields($fields, static::PREFIX['CASH']);

        return $this->walletDb->createQueryBuilder()
            ->select($fields)
            ->from('tb_user_cash_log', 'cl')
            ->where('cl_user_id = :user_id')
            ->andHaving('cl_created >= :s_date')
            ->andHaving('cl_created <= :e_date')
            ->orderBy('cl_created','DESC')
            ->setParameter('user_id', $bind['user_id'])
            ->setParameter('s_date', $bind['s_date'])
            ->setParameter('e_date', $bind['e_date'])
