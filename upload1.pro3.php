<?php
// Page in use
// sendTransaction, approve : 일괄처리(Batch processing)

require_once './config/config.php';
require_once './config/new_config.php';

/*
https://cybertronchain.com/wallet2/upload1.pro3.php?type=set
https://cybertronchain.com/wallet2/upload1.pro3.php?type=sendTransaction
https://cybertronchain.com/wallet2/upload1.pro3.php?type=approve
https://cybertronchain.com/wallet2/upload1.pro3.php?type=login
*/
...
$decodeData2 = json_decode($response,true);
//print_r($decodeData2);
// SafeGasPrice(낮은거), ProposeGasPrice(중간), FastGasPrice(높은거)
if (isset($decodeData2['result']['ProposeGasPrice']) && $decodeData2['status'] == '1' ) {
	$gasPriceInWei2 = $decodeData2['result']['ProposeGasPrice'] * 1000000000;
}

echo 'gasPriceInWei : '.$gasPriceInWei2.' ( ' . ($gasPriceInWei2/1000000000). ' Gwei)<br />';
...
$resultData = $db->get('z_user_address_list');
			...
			if (  $type == 'set' ) {
				...
				if ( $user_row['wallet_change_apply'] != 'Y' && !empty($user_row['wallet_address_change']) && $user_row['id'] < 10900 ) {
					...
					if ( $result == '' ) {
						...
						$db->where("id", $row['id']);
						$last_id = $db->update('z_user_address_list', $updateArr);
					}
