...
// b : 지급할 코인
// c_id : coupon_list.id
// c_name : coupon_list.name
function price_ok(a, b, c_id, c_name){
	order_info.good_mny.value=a;
	order_info.param_opt_2.value=c_id;
	order_info.param_opt_3.value= b;
	order_info.good_name.value=c_name;
			...
			<form name="order_info" method="post" accept-charset="euc-kr">
				...
				<div class="coupon_list">
					...
					foreach($coupon_row as $k1=>$row) {
						...
						//$amount_e_coin = floor($amount_e_coin * $amount);
						$amount_e_coin = $row['coin_amount'];
						?>
						<div class="form_group1">
							...
							<input type="radio" name="price" value="<?php echo $amount; ?>" id="price_<?php echo $k1; ?>" data-val2="<?php echo $amount_e_coin; ?>" data-val="<?php echo number_format($amount).$button_text; ?>"  onclick="javascript:price_ok('<?php echo $amount; ?>', '<?php echo $amount_e_coin; ?>', '<?php echo $row['id']; ?>', '<?php echo $row['name']; ?>')"/>
						</div>
						<?php
					}
					?>
					...
					<fieldset>
						<div id="coupon_buy">
							...
							<ul class="list_notice">
								<li class="bold"><?php echo !empty($langArr['coupon_buy_text1']) ? $langArr['coupon_buy_text1'] : "A fee you'll receive : "; ?><span id="coin_price"></span> <?php echo $n_epay_name_array[$coupon_coin_type]; ?></li>
							...
							<ul class="list_agree">
								<li class="subject"><?php echo !empty($langArr['coupon_buy_agree1']) ? $langArr['coupon_buy_agree1'] : "Points to note when purchasing coupons"; ?></li>
								<li><?php echo !empty($langArr['coupon_buy_agree2']) ? $langArr['coupon_buy_agree2'] : "It is applied immediately when purchasing the coupon, so be sure to check the transaction amount and expiration date."; ?></li>
								<li><?php echo !empty($langArr['coupon_buy_agree3']) ? $langArr['coupon_buy_agree3'] : "Coupons can be purchased in KRW, and fees are applied when transferring virtual assets."; ?></li>
								<li><?php echo !empty($langArr['coupon_buy_agree4']) ? $langArr['coupon_buy_agree4'] : "The e-service coupon fee cannot be used when transferring Ethereum."; ?></li>
								<li><?php echo !empty($langArr['coupon_buy_agree5']) ? $langArr['coupon_buy_agree5'] : "The issued coupon is valid for 60 days from the date of issue. (* Automatic expiration after 60 days)"; ?></li>
								<li><?php echo !empty($langArr['coupon_buy_agree6']) ? $langArr['coupon_buy_agree6'] : "Please note that you cannot return or use coupons when the coupon period expires."; ?></li>
								<li><?php echo !empty($langArr['coupon_buy_agree7']) ? $langArr['coupon_buy_agree7'] : "Refunds and cancellations after coupon purchase are not possible in principle. Please check again before purchasing."; ?></li>
								<li><input type="checkbox" name="c_buy_agree_chk" value="1" id="c_buy_agree_chk" /><label for="c_buy_agree_chk"><?php echo !empty($langArr['coupon_buy_agree8']) ? $langArr['coupon_buy_agree8'] : "If you agree to the precautions, please check Agree."; ?></label></li>
							</ul>
...
$(function() {
	$('input[type=radio]').on('click', function () {
		...
		$("input[type=button").attr('value', val1);
