...
$db->where('created_at', '2020-11-01 00:00:00', '>=');
//$db->where('id', '26047');

$page = 1;
$db->orderBy('rand()', 'asc');
$db->pageLimit = 1000; // 35 : 1 minutes
...
if (!empty($infos)) {
	foreach($infos as $row) {
		...
		if ( $status_r == 'none' && $row['created_at'] > $be_date ) { // fail 중 최근 60일 이내의 경우에는 실패처리 안함
			$status_r = 'F';
		}
...
function check_eth_result($txhash, $ethApiKey) {
	...
	if ( $result == '1') {
		...
		//$result2 = check_eth_result2($txhash, $ethApiKey); // pending으로 바꾸고 0x6d6ecc2d9dcde44c8a39d3151c517c50ab840f0faebf9284e092648805c6f559 테스트해보기
		//$status = $result2;
		$status = 'pending';
	}
	return $status;
