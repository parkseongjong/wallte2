...
$log->info('token 목록 > eth send 조회',['target_id'=>0,'action'=>'S']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
	...
	new_fn_logSave( 'Message : (' . $_SESSION['user_id'] . ', OJT TEST eth /value :'.filter_input(INPUT_POST, 'amount', FILTER_SANITIZE_SPECIAL_CHARS).' / hexValue :'.$amountToSend.' / before gas : '.$beForeGas.' / hex gas : '.$gasPriceInWei.')');
	// 20.08.04
	if ( $getNewBalance <= $filterData['amount'] ) {
	...
	try {
		...
		new_fn_logSave( 'Message : (' . $_SESSION['user_id'] . ', eth /value :'.filter_input(INPUT_POST, 'amount', FILTER_SANITIZE_SPECIAL_CHARS).' / hexValue :'.$amountToSend.') ' . $e->getMessage() . ', Code : ' . $e->getCode() . ', File : ' . $e->getFile() . ' on line ' . $e->getLine());
	...
	if(!empty($feeTransactionId)){
		...
		$toMemberId = $db->where("wallet_address",$toAccount)->getOne('admin_accounts','id');
		// if(!$toMemberId){
		// 	$toMemberId = '';
		// }
		if(empty($toMemberId)){
		...
		$data_to_send_logs['to_id'] = $toMemberId['id'];
		...
		//token general log 기록 by.ojt 2021.07.31 START
		$log->info('token 목록 > eth 보내기 > eth 전송',['target_id'=>$toMemberId['id'],'action'=>'A']);
		//token general log 기록 END
