...
$web3bsc=$web3Instance->bscChainInnerInit();
...
if($search_string) {
    ...
    if (preg_match("/[\xE0-\xFF][\x80-\xFF][\x80-\xFF]/", $search_string)){ // utf-8인 경우
        ...
        $db->orWhere('n_phone','%'.$search_string.'%','like');
        /*
        ...
        $db->orWhere('n_phone',$search_string);
        */
    }
    //OK = 가능 NONE = 불가능
    if(!empty($loginOrNot)) {
        if($loginOrNot == 'OK'){
            $db->where('login_or_not','Y');
        }
        else{
            $db->where('login_or_not','N');
        }
    }
...
<style>
    ...
    <link href="css/admin.css?v=20220617" rel="stylesheet">
    ...
    <div id="page-wrapper" >
        ...
        <div class="well text-center filter-form">
            <form class="form form-inline" action="">
                ...
                login:
                <select name="loginOrNot">
                    <option value="OK" selected>가능</option>
        ...
        <!--   Filter section end-->
        <hr>
        <!-- 2021.11.04 엑셀 다운로드 기능 비활성화 by. OJT -->
        <!--	<a class="btn btn-success" href="admin_users_export.php">Download CSV File</a> | -->
<!--        	<a class="btn btn-success" href="admin_users_export2.php?date1=--><?php //echo !empty($date1) ? $date1 : ''; ?><!--&date2=--><?php //echo !empty($date2) ? $date2 : ''; ?><!--&admin_type=user--><?php //echo isset($search_string)?'&search_string='.$search_string:''?><!--">-->
<!--                Download CSV File(new) : User-->
<!--            </a>-->
        <hr>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered admin_table_new">
                        ...
                        <tr>
                            ...
                            <th><?php echo !empty($langArr['swap_block']) ? $langArr['swap_block'] : "Block For Swap"; ?></th>
                            ...
                            <th>CTCTM<!--<?php echo !empty($langArr['ctctm_balance']) ? $langArr['ctctm_balance'] : "CTCTM Balance"; ?>--></th>
                        ...
                        foreach ($resultData as $row) {
                            ...
                            if ( !empty($row['wallet_address']) && strlen($row['wallet_address']) > 10 ) {
                                ...
                                $userCtcTmAmt = getMyBnbTokenBalance($row['wallet_address'],$tokenPayAbi,$contractAddressArr['ctctm']['contractAddress'],$contractAddressArr['ctctm']['decimal'], $n_connect_ip, $n_connect_port); 
                            ...
                            <tr>
                                ...
                                <td rowspan="2"><?php echo $row['swap_block']; ?></td>
                                ...
                                <td>
                                    <a href="token_adm.php?token=ctctm&user_id=<?php echo $row['id']; ?>" target="_blank"><?php echo new_number_format($userCtcTmAmt, $n_decimal_point_array['ctctm']); ?></a>
                                ...
                                <td rowspan="2">
                                    ...
                                    /*
                                        echo $row['device'].'<br />
                                        (C)'.$walletMasking->reset()->pushEmailMask()->pushUniversalIdMask()->getMasked(htmlspecialchars($row['devId'])).'<br />
                                        (B)'.$row['devId2'];
                                    */
                                    ?>

                                    <?php echo $row['device'] ?><br />
                                    <?php echo '(C)'.(empty($row['devId'])?'디바이스 ID 없음':'디바이스 ID <b>있음</b>') ?><br />
                                    <?php echo '(B)'.(empty($row['devId2'])?'디바이스 ID 없음':'디바이스 ID 있음') ?><br />
...
function getMyBnbTokenBalance($address,$testAbi,$contractAddress,$setDecimal, $n_connect_ip, $n_connect_port){

    global $web3Inner, $web3Instance,$web3bsc;
