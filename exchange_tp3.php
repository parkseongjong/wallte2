...
$db = getDbInstance();
$db->where("module_name", 'exchange_rate_tp3');
...
try {
	...
	$contract = new Contract($web3->provider, $contractAddressArr[$token]['abi']);
	$contract->at($contractAddressArr[$token]['contractAddress'])->call($functionName, $walletAddress,function($err, $result) use (&$getNewCoinBalance, &$decimal){
		if ($err !== null) {
			throw new Exception($err->getMessage(), 2);
		}
		if ( !empty( $result ) ) {
			$getNewCoinBalance = reset($result)->toString();
			$getNewCoinBalance = $getNewCoinBalance/$decimal;
		}
	});
} catch (Throwable $e) {
	new_fn_logSave( 'Message : ' . $e->getMessage() . ', Code : ' . $e->getCode() . ', File : ' . $e->getFile() . ' on line ' . $e->getLine());
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($_SESSION['user_id']==$n_master_id_exc_tp3){ // 45, Changed it to set it at once on that page : config/new_config.php
		$walletAddress = $row[0]['wallet_address'];
	}else{
	...
	$db = getDbInstance();
	$db->where("module_name", 'min_exchange_amount_tp3');
	$getMinAmount = $db->getOne('settings');
	$getMinAmountVal = !empty($getMinAmount['value']) ? $getMinAmount['value'] : '0.01';
	...
	$coinBalance = 0;
	try {
		$contract->at($contractAddressArr[$token]['contractAddress'])->call($functionName, $n_master_wallet_address_exc_out_tp3,function($err, $result) use (&$coinBalance, &$decimal){
			if ($err !== null) {
	...
	if($coinBalance<$ctcAmountToSend){
		$_SESSION['failure'] = !empty($langArr['exchange_message1_tp3']) ? $langArr['exchange_message1_tp3'] : 'Insufficient TP3 Balance in Admin Account.';
		header('location: '.$return_page);
		exit();
	}
	...
	if($_SESSION['user_id']==$n_master_id_exc_tp3){ // 45, Changed it to set it at once on that page : config/new_config.php
		...
		$toAccount=$n_master_wallet_address_exc_tp3;
		...
		try {
			...
			], function ($err, $transaction) use ($eth, $fromAccount, $toAccount, &$actualAmountToSendWithoutDecimal,&$ethTransactionId,&$token) {
				if ($err !== null) {
		...
		$msg = "Transaction has made:) id: <a href=https://etherscan.io/tx/".$ethTransactionId.">" . $ethTransactionId . "</a>. ".$langArr['exchange_message5'];
		...
		$data_to_send_logs['to_id'] = $n_master_id_exc_tp3;
		...
		// send CTC Token To User Account
			...
			$personal = $web3->personal;
			$personal->unlockAccount($fromAdminAccount, $n_master_wallet_pass_exc_tp3, function ($err, $unlocked) {
			...
			if(!empty($transactionId)){
				...
				$data_to_store = [];
				$data_to_store['created_at'] = date('Y-m-d H:i:s');
				$data_to_store['sender_id'] = $n_master_id_exc_tp3;
						...
						<div class="card">
							<ul class="index_token_block">
								<li class="token_block">
									<a href="token.php?token=<?php echo $token; ?>" title="<?php echo strtoupper($token); ?>">
								...
								<form class="form" action="" method="post"  id="customer_form" enctype="multipart/form-data">
									<div class="form-group col-md-12">
										<label class="address_area">
											...
											<span class="fee1"><?php echo !empty($langArr['exchange_rate']) ? $langArr['exchange_rate'] : "Exchange Rate :"; ?>  1 ETH  = <?php echo $getExchangePrice; ?> <?php echo strtoupper($token); ?></span>
										</label>
									...
									<div class="form-group col-md-12">
										<label><span class="label_subject"><?php echo !empty($langArr['tp3_amount']) ? $langArr['tp3_amount'] : "TP3 Amount :"; ?></span></label>
										<input autocomplete="off" disabled required id="tp3amount" name="tp3amount" placeholder="" type="text">
									</div>
   ...
   $("#customer_form").validate({
    ...
    if($(this).val() == '')
        ...
        else
        {
			...
			var tp3amt = getAmt*<?php echo $getExchangePrice; ?>;
			$("#tp3amount").val(tp3amt);
            
        }
    });

    $('#tp3amount').keyup(function () {
    if($(this).val() == '')
        {
            $("#ethamount").val(0);
        }
