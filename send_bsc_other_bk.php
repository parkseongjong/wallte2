...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
	...
	if(!empty($feeTransactionId)){
		try {
            ...
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS =>'{"sender_pvt_key":"'.$getUserPvtKey.'","to_address":"'.$toAccount.'","amount":'.$amountToSendApi.',"token":"'.$token.'"}',
            CURLOPT_HTTPHEADER => array(
                'Content-Type: application/json'
            ),
            ));
							...
							<div class="boxed bg--secondary boxed--lg boxed--border">
								...
								<form class="form" action="set_transferpw_frm_send.php" method="post"  id="customer_form" enctype="multipart/form-data">
									...
									<div class="form-group col-md-12">
										...
										<input autocomplete="off"  required title="<?php echo $langArr['this_field_is_required']; ?>" onkeyup="test()" oninput='handleOnInput(this, 5)' id="amount" name="amount" placeholder="<?php echo !empty($langArr['send_explain2']) ? $langArr['send_explain2'] : 'Please enter the quantity to send.'; ?>" type="number" value="0">
                                    ...
                                    ?>
                                            <div class="col-md-12 btn_area">
                                                <input name="submit" class="btn" id="confirm_modal" value="<?php echo !empty($langArr['send_amount']) ? $langArr['send_amount'] : "Send Amount"; ?>" type="submit" onclick="end_coin();" />
                                                <script>
                                                    function end_coin(){
                                                        alert("전송량이 초과하였습니다.");
                                                        return;
