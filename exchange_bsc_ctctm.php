...
$return_page = 'exchange_bsc_ctctm.php';
...
//die("2");
	...
	if(($getCtctmTotalSum+$bscAmount) > $getMaxCtctmSwapPerUserAmt){
		$_SESSION['failure'] = !empty($langArr['swap_max_limit_message2']) ? $langArr['swap_max_limit_message2'] : ' Max Limit Exceed : Already Swapped - '.$getCtctmTotalSum;
		header('location: '.$return_page);
		exit();
	}
	...
	$getNewCoinBalance = $wi_wallet_infos->wi_get_balance('2', $ethToken, $walletAddress, $contractAddressArr);
...
*/
	...
	if(!empty($feeTransactionId)){
		...
		if(!empty($transactionId)) {
			...
			$_SESSION['failure'] = !empty($langArr['send_message222']) ? $langArr['send_message222'] : "Unable to send Token. Try Again22.";
			header('location: '.$return_page);
			exit();
		}
	}
