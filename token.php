...
switch ($tokenName) {
	...
	case 'ctc':
		...
		try {
			...
			if($tokenName=='ctc7' || $tokenName=='ctctm'){
				$otherTokenContract = $web3Instance->innerContract($web3bsc->provider, $tokenAbi);
			}else{
...
if ($tokenName == 'ctc') {
    //CTC 테스트 페이지로 연동 테스트용
	$sendPageUrl = "send_token.php";
   // $sendPageUrl = "send_token_mytest.php";
} else if ($tokenName == 'eth') {
	$sendPageUrl = "send_eth.php";
} else if ($tokenName == 'ctctm' || $tokenName == 'ctc7') {
	$sendPageUrl = "send_bsc_other.php?token=".strtolower($tokenName);
} else {
	...
	<div id="token">
		...
		<div class="history_top">
			<div class="top">
				...
				<ul>
					<?php if ( $_SESSION['user_id'] != '5137' && ( $tokenName == 'ctc' || $tokenName == 'tp3' || $tokenName == 'mc') ) { ?>
					...
					<?php } else { ?>
                        ...
                        <script>
                            function day_end(){
                                alert("It's under temporary inspection.");
                            }
                        </script>
                        <?php if($_SESSION['user_id'] == '17417' || $_SESSION['user_id'] == '93') { ?>
                    <!--    <li class="send_btn"><a href="<?php /*echo $sendPageUrl; */?>" title="send"><?php /*echo !empty($langArr['send']) ? $langArr['send'] : "Send"; */?></a></li>-->
                            <?php }?>
...
//function view_message () {
//    let msg = "<?php //echo !empty($langArr['commonServiceNotice']) ? $langArr['commonServiceNotice'] : "Checking. We will complete the inspection as soon as possible."; ?>//";
//    bsCommonAlert(msg);
//}
...
function get_token_history(page, view_type) {
	...
	var mode = '<?php echo ($tokenName=="ctctm" || $tokenName=="ctc7") ? "get_bsc_history"  : "get_token_history2"  ?>';
	$.ajax({
		url : 'send.pro.php',
		type : 'POST',
		data : {mode: mode, waddr : waddr, token : token, page : page, waddr2 : waddr2},
